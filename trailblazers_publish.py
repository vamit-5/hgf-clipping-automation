...
# ---------------------------------------------------------------------------
# Trailblazers x Ann Miura-Ko - PRAVA OBJAVA na Instagram. Ista logika kao
# trailblazers_test.py (WeTransfer download, transkripcija, hook-detekcija,
# supercut generisanje), ali se svaki od N_HOOKS klipova STVARNO objavljuje
# na Instagram (Cloudinary upload + IG Graph API), jedan za drugim, sa pauzom
# izmedju objava.
# ---------------------------------------------------------------------------
...
DELAY_BETWEEN_POSTS_SECONDS = 180
...
def main():
    ...
    with open("hooks_debug.json", "w", encoding="utf-8") as f:
    ...
    for idx, hook in enumerate(hooks):
        ...
        published.append({"idx": idx, "media_id": media_id, "caption": hook["caption"]})

        with open("published_debug.json", "w", encoding="utf-8") as f:
            json.dump(published, f, indent=2, ensure_ascii=False)

        if idx < len(hooks) - 1:
            print(f"Cekam {DELAY_BETWEEN_POSTS_SECONDS}s pre sledece objave...")
            time.sleep(DELAY_BETWEEN_POSTS_SECONDS)

    print(f"SVI KLIPOVI OBJAVLJENI NA INSTAGRAM. Ukupno: {len(published)}.")
