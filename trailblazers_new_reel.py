...
# - titlovi ispod vertikalne sredine (ne prekrivaju lica, nisu previse dole)
...
#   pise "Trailblazers Podcast | Ann Miura-Ko")
...
def build_captions_file(words, clip_start, clip_end, path):
    ...
    # MarginV=750 na PlayResY=1920 platnu -> titlovi sede ispod vertikalne
    # sredine, ne prekrivaju lica, i nisu previse pri dnu da postanu nevidljivi.
    header = """[Script Info]
ScriptType: v4.00+
PlayResX: 1080
PlayResY: 1920
...
def finalize_clip(input_path, captions_path, output_path, background_audio_path=None):
    """Ostar, PUN 9:16 kadar - BEZ blur pozadine i letterboxing-a (drop split/
    gblur/overlay iz stare verzije). Samo scale+crop koji vec dobro drzi oba
    lica u kadru (potvrdjeno od korisnika), plus mali brending natpis na vrhu
    (Trailblazers Podcast + Ann Miura-Ko - branding/title rules iz brief-a),
    titlovi, i po zelji glasnija dramaticna pozadinska muzika."""
    filter_parts = [
        "[0:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920[cropped];",
        "[cropped]drawtext=text='TRAILBLAZERS PODCAST  |  Ann Miura-Ko':"
        "fontcolor=white:fontsize=42:box=1:boxcolor=black@0.55:boxborderw=14:"
        "x=(w-text_w)/2:y=80[branded];",
        f"[branded]ass={captions_path}[outv];",
    ]

    cmd = ["ffmpeg", "-y", "-i", input_path]

    has_bg = bool(background_audio_path) and os.path.exists(background_audio_path)
    if has_bg:
        cmd += ["-stream_loop", "-1", "-i", background_audio_path]
        filter_parts.append(
            "[0:a]volume=1.0[dlg];"
            f"[1:a]volume={BACKGROUND_AUDIO_VOLUME}[bgaud];"
    ...
    print("Zavrsna obrada (ostar pun 9:16 kadar, brending, titlovi" + (", muzika" if has_bg else "") + ")...")
    result = subprocess.run(cmd, capture_output=True, text=True, timeout=FFMPEG_TIMEOUT)
    if result.returncode != 0:
        print(result.stderr[-3000:])
...
def main():
    ...
    captions_path = "output_clips_newreel/captions.ass"
    ...
    finalize_clip(supercut_path, captions_path, final_path, background_audio_path=chosen_background_audio)
